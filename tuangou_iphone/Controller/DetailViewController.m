...
- (IBAction)collectBtnClick:(UIButton *)sender {
    ...
    if (sender.selected == YES) {
        ...
        [Utils removeObjectFromCollection:_dict];
        count = (int)[Utils getCollection].count;
        NSLog(@"%@",[Utils getCollection]);
        NSLog(@"收藏的数量为%d",count);
    }else{
        [sender setSelected:YES];
       [Utils addObjectToCollection:_dict];
        count = (int)[Utils getCollection].count;
        NSLog(@"%@",[Utils getCollection]);
         NSLog(@"收藏的数量为%d",count);
    }
